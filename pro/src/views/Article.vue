<template>
  <div>
    ...
    <div id="content" @click="doWritex">
      <!-- 标题文章简介开始 -->
      <p>
        <!-- <span>校园霸凌页面测试数据！ 此处是文章简介部分， 此处是文字加图片组合成
          的页面文章内容简介部分！ -->
        <span>
          {{ articleData.title }}
      ...
      <div id="textStart">
        ...
        <div id="msgx">
          ...
          <div>
            ...
            <span>{{ articleData.createTime | articleTime }}</span>
            <span>-</span>
    ...
    <div class="comDis">
      ...
      <div id="comFloor2" v-for="(item,index) in answerData" :key="index">
        ...
        <div id="comMain">
          ...
          <!-- 评论内容 -->
          <h3>{{ item.content }}</h3>
          <p>{{ item.createdTime | articleTime }}</p>
        </div>
...
export default {
  ...
  methods: {
    //公开评论
    doAnswer() {
      let url = `/api/bully-publish/publish/answers/model/${this.aid}?page=1`;
      console.log('urlArt',url);
      this.$axios.get(url).then((res) => {
        // console.log("answerArticle", res);
        this.answerData = res.data.data.list;
        console.log("answerArticle",this.answerData);
      });
    },

    // 个人页面详情
    articleNews() {
      let url = `/api/bully-publish/publish/models/${this.aid}`;
      this.$axios.get(url).then((res) => {
        this.articleData = res.data.data;
        console.log("articleNews",this.articleData);
      });
    },
  },
  ...
  // 评论区点赞
  doComAssist(index) {
    this.comAssist = this.comAssist == null ? index:null;
  },
};
</script>
